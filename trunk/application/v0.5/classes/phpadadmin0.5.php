...
var $questionstoask=3; // Number of questions to ask a user
...
var $version="0.5";
	...
	function dbquery($sql,$format="numbered") // output a mysql query to an array format ( Numbered Names or Both)
		{
		$this->dbconnect();
	...
	function getdbfunctions()
		{
		$output=$this->dbquery("SELECT * FROM `functions` WHERE `enable` = '1'");
		return($output);
		}
	function getdbfields($format="numbered")
		{
		$output=$this->dbquery("SELECT * FROM `fields` WHERE enabled = '1'",$format);
		return($output);
		}
	function getdbuserid($username,$domain)
		{
		$this->dbconnect();
		$sqluser=$username."@".$this->domainconfig[$domain]['fqdn'];
